...
class TestTranslationIntegration:
    ...
    def mock_translation_tools(self):
        ...
        mock_tools.generate_search_queries.return_value = [
            "Trương Tam", "lập trình viên", "trí tuệ nhân tạo"
        ]
        
        # Mock translation
        mock_tools.translate_chunk.return_value = "Đây là bản dịch thành công của đoạn văn."
        ...
        mock_tools.generate_context_summary.return_value = "Tóm tắt: Nhân vật chính chuẩn bị cho cuộc họp quan trọng."
        mock_tools._format_recent_context.return_value = "Ngữ cảnh gần đây: Nhân vật chính"
        
        return mock_tools
    
    def test_complete_translation_workflow(self, config, sample_input_text, mock_weaviate_client, mock_translation_tools):
    ...
    def test_translation_workflow_with_existing_memory(self, config, sample_input_text, mock_weaviate_client, mock_translation_tools):
        ...
        with patch('agent.graph.WeaviateWrapperClient', return_value=mock_weaviate_client), \
            ...
            assert any('Ngữ cảnh trước đó' in item['summary'] for item in final_state['memory_context'])
    
    def test_translation_workflow_error_recovery(self, config, sample_input_text, mock_weaviate_client, mock_translation_tools):
    ...
    def test_memory_consistency_across_chunks(self, config, sample_input_text, mock_weaviate_client, mock_translation_tools):
    ...
    def test_context_overflow_handling_integration(self, config, sample_input_text, mock_weaviate_client, mock_translation_tools):
    ...
    def test_translation_quality_tracking(self, config, sample_input_text, mock_weaviate_client, mock_translation_tools):
        ...
        with patch('agent.graph.WeaviateWrapperClient', return_value=mock_weaviate_client), \
            ...
            assert final_state['translation_state']['processing_status'] in ['completed', 'failed']
    
    def test_memory_search_efficiency(self, config, sample_input_text, mock_weaviate_client, mock_translation_tools):
    ...
    def test_final_output_quality(self, config, sample_input_text, mock_weaviate_client, mock_translation_tools):
